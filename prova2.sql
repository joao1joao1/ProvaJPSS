...
select nome, data_contratacao from Medicos
order by data_contratacao asc;

select nome, especializacoes_adicionais from Medicos
where especializacoes_adicionais like '%Cirurgia%';

select nome, email from Medicos
order by nome asc;

select nome, horario_trabalho from Medicos
where horario_trabalho = '12:00 - 20:00' or horario_trabalho = '11:00 - 19:00';

select nome, telefone from Medicos
where telefone like '%8901';

select nome, salario from Medicos where salario between 15000 and 16000;

select round(avg(salario),2) as 'média_salario_all' from Medicos;